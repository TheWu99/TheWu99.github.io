...
function inline(text) {
  ...
  value = value.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, function (_, alt, url) { return '<img src="' + escapeHtml(safeUrl(url)) + '" alt="' + alt + '" loading="lazy">'; });
  value = value
...
async function loadChapter(number) {
  ...
  document.title = chineseChapter(number) + '：' + chapter.title + ' · 透过地理看历史';
...
function setTheme(theme) {
  ...
  localStorage.setItem('history-geography-theme', theme);
...
function cycleFontSize() {
  ...
  localStorage.setItem('history-geography-font-size', next);
...
async function init() {
  const savedTheme = localStorage.getItem('history-geography-theme');
  const preferredDark = matchMedia('(prefers-color-scheme: dark)').matches;
  setTheme(savedTheme || (preferredDark ? 'dark' : 'light'));
  root.dataset.fontSize = localStorage.getItem('history-geography-font-size') || 'medium';
